...
func NewIncrementCommand() *cobra.Command {
	...
	incrementCmd.Flags().StringVarP(&config.incrementType, "type", "t", string(models.Patch), "The type of increment to perform, options: major, minor, patch")
	...
	incrementCmd.Flags().StringVarP(&config.sourceVersions, "source-versions", "V", "", "The source versions to increment from")
